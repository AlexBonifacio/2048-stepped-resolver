...
def outcome_summary():
    ...
    for path in SESSION_DIR.glob("*.json"):
        ...
        target = int_value(outcome.get("target"), 13)
        sessions.append({
            "session": path.stem,
            "success": success,
            "target": target,
...
class Handler(BaseHTTPRequestHandler):
    ...
    def handle_get_suggestion(self, parsed):
        ...
        quality = query.get("quality", ["strong"])[0]
        model_session = query.get("model_session", ["youtube_max01_VozVoz"])[0]
        model_stats = query.get("model_stats", ["session"])[0]
        target = query.get("target", ["13"])[0]
        try:
        ...
        if solver not in ("expectimax", "optimistic", "human", "hybrid", "target"):
            self.write_json({"ok": False, "error": "Solveur invalide."}, status=400)
            return
        if quality not in ("fast", "balanced", "strong", "godlike"):
            self.write_json({"ok": False, "error": "Qualite invalide."}, status=400)
            return
        if model_stats not in ("global", "session", "merged"):
            self.write_json({"ok": False, "error": "Source de stats invalide."}, status=400)
            return
        if not SOLVER_BIN.exists():
            self.write_json({"ok": False, "error": "Binaire 2048-ranks introuvable. Lance make."}, status=500)
            return
        ...
        try:
            ...
            self.write_json({"ok": False, "error": f"Calcul trop long apres {timeout}s."}, status=504)
            return
...
def main():
    ...
    url = (
        ...
        "&solver=hybrid&quality=strong&model_session=youtube_max01_VozVoz"
        "&model_stats=session&target=13&timeout=60"
    )
